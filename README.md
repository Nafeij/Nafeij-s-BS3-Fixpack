# Nafeij's BS3 Fixpack - version 1.4
...
> ## 1.4
> 5 April 2026
> - Fixed animation bug that occurs when a hero equipped with an item that grants Folka's "Shield Mastery" passive loses all their armor. The game will try to set the 'noshield' layer for the hero, which only exists for Folka; this will cause the hero to turn invisible. Fixed with an explicit check for the hero's ID in `engine\battle\ability\effect\op\model\Op_SetAnimLayer.as`.
...
> - If one of your heroes is possessed by Eyeless, defeating your possessed hero grants you a duplicate of whatever item they had equipped. Fixed by adding a check for entities with a `"POSSESSED"` tag, in `engine\battle\entitiy\model\BattleEntity._handleDeath_pre`, to prevent them from dropping their items.
> - The passive on all types of Dredge Hurlers, Distraught, isn't triggering properly. Fixed by changing `responsePhase` to `POST_COMPLETE `in its corresponding entry in `_ability.index.json`.
...
> - Miscellaneous typos.
> - <details><summary>Click to reveal spoiler</summary>Juno dialogue branch in her history lesson was dropped in a previous version of the mod. This has been added back in.</details>
> - <details><summary>Click to reveal spoiler</summary>Swapped out Valgard with Folka (if she is alive) in the conversation before recovering Juno's body.</details>
...
> - Many, many typos and broken dialogue trees in conversations, popups and cut-scene subtitles.
...
> - Survival finale cut-scene and final scores not triggering properly.
> - Gave Stonesingers +1 range.
> - Added several flags to un-equip items when characters are removed from the roster.
> - Secret item from Yak's BS2 Fixpack included for continuity.
> - <details><summary>Click to reveal spoiler</summary>Sprites are positioned too high where it shows the Ravens walking atop the white tower.</details>
> - <details><summary>Click to reveal spoiler</summary>Mogun says a line that makes no sense if Hogun dies in Saga 1, or if he hangs his axe in Saga 2.</details>
> - <details><summary>Click to reveal spoiler</summary>Bug in storyline where it doesn't say Iver and the Ravens return to Arberrang if Oddleif is dead.</details>
