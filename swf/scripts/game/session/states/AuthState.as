package game.session.states
{
   ...
   public class AuthState extends GameState
   {
      ...
      private static const LOCAL_BUILD_NUMBER:String = "local-build-number";
      ...
      private function authCallbackHandler(param1:AuthTxn) : void
      {
         ...
         if(param1 && param1.success && Boolean(param1.jsonObject))
         {
            ...
            if(param1.buildNumber != config.context.appInfo.buildNumber)
            {
               logger.info("Server build number is " + param1.buildNumber + ", but ours is " + config.context.appInfo.buildNumber + ", seeking confirmation");
               if(config.context.appInfo.buildNumber != LOCAL_BUILD_NUMBER && param1.buildNumber != LOCAL_BUILD_NUMBER)
               {
                  this.action = null;
