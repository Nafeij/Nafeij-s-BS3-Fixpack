package game.gui.pages
{
   ...
   public class GuiPgDetails_KillCounter
   {
      ...
      public function updateKillCounter(param1:IEntityDef, param2:IGuiContext) : void
      {
         ...
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         if(Boolean(param2.saga) && param2.saga.isEternalArena)
         {
            _loc3_ = param2.saga.getMasterKillCountForEntity(param1.eaKillCounterId);
            this.killCounterText.htmlText = "<font color=\"" + this.killGoldTextColor + "\">" + _loc3_ + "</font>";
         }
         else
         {
            _loc4_ = int(param1.stats.getValue(StatType.RANK));
            _loc5_ = int(param1.stats.getValue(StatType.KILLS));
            _loc6_ = param1.statRanges.getStatRange(StatType.RANK).max;
            if(_loc4_ >= _loc6_ && _loc4_ > 1)
            {
               this.skulls_gold.visible = true;
               this.skulls.visible = false;
               this.killCounterText.htmlText = "<font color=\"" + this.killGoldTextColor + "\">" + _loc5_ + "</font>";
            }
            else
            {
               this.skulls_gold.visible = false;
               this.skulls.visible = true;
               _loc7_ = param2.statCosts.getKillsRequiredToPromote(_loc4_);
               this.killCounterText.htmlText = "<font color=\"" + this.currentKillTextColor + "\">" + _loc5_ + "</font>" + "<font color=\"" + this.killsToPromoteTextColor + "\">/" + _loc7_ + "</font>";
            }
         }
      }
   }
