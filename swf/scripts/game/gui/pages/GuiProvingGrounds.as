package game.gui.pages
{
   ...
   public class GuiProvingGrounds extends GuiBase implements IGuiProvingGrounds
   {
      ...
      public function init(param1:IGuiContext, param2:GuiProvingGroundsConfig, param3:IGuiProvingGroundsListener, param4:IGuiPgAbilityPopup) : void
      {
         ...
         var _loc6_:SagaEternalArena = !!_loc5_ ? _loc5_.eternalArena : null;
         if(!param2.allowRenown || _loc5_ && _loc5_.isSurvivalSettingUp || Boolean(_loc6_))
         {
         ...
         this.setRenown();
         var _loc7_:ILegend = param1.legend;
         if(_loc7_)
         {
            if(_loc7_.roster.numCombatants > 0)
            {
               this.selectedCharacter = _loc7_.roster.getEntityDef(0);
            }
            _loc7_.addEventListener(LegendEvent.RENOWN,this.renownHandler);
         }
         else
         {
         ...
         var _loc8_:MovieClip = this.questionPages.getChildByName("question_renown") as MovieClip;
         if(_loc8_)
         {
            this._helpRenownOriginalY = _loc8_.y;
         }
         this.updateGpIcons();
      }
      ...
      private function setupButtonTown() : void
      {
         ...
         else if(Boolean(_loc1_) && Boolean(_loc1_.eternalArena))
         {
      ...
      private function onQuestionClick(param1:ButtonWithIndex) : void
      {
         ...
         if(this.questionPages.parent == null)
         {
            ...
            var _loc2_:DisplayObject = this.questionPages.getChildByName("respec_help");
            var _loc3_:DisplayObject = this.questionPages.getChildByName("kills_help");
            var _loc4_:DisplayObject = this.questionPages.getChildByName("kills_help_ea_alt");
            if(_loc2_)
            {
               _loc2_.visible = _context.saga.isEternalArena && Boolean(this._details.selectedCharacter) && this._details.selectedCharacter.respecAvailable;
            }
            if(_loc3_)
            {
               _loc3_.visible = !_context.saga.isEternalArena;
            }
            if(_loc4_)
            {
               _loc4_.visible = _context.saga.isEternalArena;
            }
            ...
            if(Platform.requiresUiSafeZoneBuffer)
            {
               _loc5_ = this.questionPages.getChildByName("question_renown") as MovieClip;
               if(_loc5_)
               {
                  _loc5_.y = this._helpRenownOriginalY + 20;
               }
            }
            scaleTextfields();
      ...
      private function onDisplayCharacterDetails(param1:GuiRosterEvent) : void
      {
         ...
         if(this._pg_top_banner.parent == this.rosterComponent)
         {
            ...
            this.setChildIndex(this._pg_top_banner,3);
         }
         if(this.buttonTown)
         {
            this.setChildIndex(this.gp_b,this.numChildren - 1);
