package game.gui
{
   ...
   public class GuiConvoBase extends GuiBase implements IGuiConvo
   {
      ...
      public function appendText(param1:String, param2:String, param3:String) : void
      {
         if(!param3)
         {
            context.logger.error("CONVO [Missing text] " + this._cursor.nodeIds);
            param3 = this._cursor.nodeIds;
         }
      ...
      public function showConvoCursor(param1:ConvoCursor) : void
      {
         ...
         if(param1.hasOptions)
         {
            ...
            for each(_loc7_ in param1.options)
            {
               ...
               if(!_loc8_)
               {
                  _loc8_ = "NO OPTION TEXT FOR " + _loc7_.nodeId + " " + _loc6_;
                  context.logger.error("ConvoCursor.showConvoCursor unable to get text for option " + _loc7_.nodeId);
               }
