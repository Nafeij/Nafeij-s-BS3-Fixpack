package game.gui.page
{
   ...
   public class ScenePageBattleHandler
   {
      ...
      private function resultsPageClosedHandler(param1:MatchResolutionPage) : void
      {
         ...
         var _loc4_:EternalArenaArrangement = null;
         var _loc5_:EternalArenaArrangement = null;
         var _loc6_:StateData = null;
         ...
         if(_loc2_)
         {
            ...
            else if(_loc2_.isEternalArena)
            {
               if(this.page.config.fsm.current.data.getValue(GameStateDataEnum.REMATCH))
               {
                  this.page.config.fsm.current.data.setValue(GameStateDataEnum.REMATCH,false);
                  _loc3_ = _loc2_.eternalArena;
                  _loc4_ = _loc3_.currentArrangement;
                  if(_loc4_)
                  {
                     _loc2_.eternalArena.playBattle(_loc4_,_loc3_.isChallengeArrangementActive);
                     return;
                  }
                  this.logger.e("ETAR","Attempting to replay Arena Battle with invalid arrangement! Returning to Arena Setup.");
               }
               else if(this.page.config.fsm.current.data.getValue(GameStateDataEnum.EA_EDIT_CURRENT_ARENA))
               {
                  _loc5_ = _loc2_.eternalArena.currentArrangement;
                  if(_loc5_)
                  {
                     if(_loc2_.eternalArena.isChallengeArrangement)
                     {
                        _loc5_ = new EternalArenaArrangement().fromJson(_loc5_.toJson());
                        _loc5_.hash = _loc2_.eternalArena.currentArrangement.hash;
                     }
                     _loc6_ = new StateData();
                     _loc6_.setValue(GameStateDataEnum.EA_ARRANGEMENT,_loc5_);
                     _loc6_.setValue(GameStateDataEnum.EA_ARRANGEMENT_READONLY,false);
                     this.page.config.fsm.transitionTo(SagaEternalArenaSetupState,_loc6_);
                     return;
                  }
               }
               this.page.config.fsm.transitionTo(SagaEternalArenaStartState,null);
               return;
            }
         }
         if(this._focusedBoard)
         {
            ...
            _loc7_ = this._focusedBoard.sim.fsm.current;
            if(_loc7_)
            {
               _loc7_.data.setValue(GameStateDataEnum.SCENE_LOADER,null);
               _loc7_.data.setValue(GameStateDataEnum.LOCAL_PARTY,null);
               _loc7_.data.setValue(GameStateDataEnum.PLAYER_ORDER,null);
               _loc7_.data.setValue(GameStateDataEnum.OPPONENT_NAME,null);
               _loc7_.data.setValue(GameStateDataEnum.OPPONENT_ID,null);
               _loc7_.data.removeValue(GameStateDataEnum.BATTLE_INFO);
            }
