   ...
   public class SagaOptionsPage extends GamePage implements IGuiSagaOptionsListener, IGuiSagaOptionsDifficultyListener, IGuiOptionsLangListener, IGuiOptionsGpListener, IGuiOptionsBattleObjectivesListener, IGuiOptionsAudioListener
   {
      ...
      public function guiOptionsQuitGame() : void
      {
         ...
         if(this.saga)
         {
            ...
            if(this.saga && this.saga.sound && Boolean(this.saga.sound.system))
            {
               this.saga.sound.system.stopVo();
            }
            ...
            else
            {
               ...
               else if(this.saga.eternalArena)
               {
                  if(this.saga.getBattleBoard())
                  {
                     this.saga.eternalArena.gaReportBattleEnd(false);
                     config.fsm.transitionTo(SagaEternalArenaStartState,null);
                  }
                  else
                  {
                     config.loadSaga(this.saga.parentSagaUrl,null,null,-1,-1);
                  }
                  return;
               }
               if(this.saga.canSave(null,null))
