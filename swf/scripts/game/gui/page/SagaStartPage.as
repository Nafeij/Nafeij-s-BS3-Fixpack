package game.gui.page
{
   ...
   public class SagaStartPage extends GamePage implements IGuiSagaStartListener
   {
      ...
      private var _etar:SagaStartPage_EternalArena;
      ...
      override protected function handleLoaded() : void
      {
         ...
         if(Boolean(fullScreenMc) && !this.gui)
         {
            ...
            _loc4_ = config.saga;
            _loc2_ = SagaDef.SURVIVAL_ENABLED && !_loc4_.def.survival;
            _loc3_ = SagaDef.ETERNAL_ARENA_ENABLED && !_loc4_.def.eternalArena && Boolean(_loc4_.def.eternalArenaSagaUrl);
            fullScreenMc.visible = false;
            this.gui.init(config.gameGuiContext,this,this._resumeCount > 0,this._hasCheckpoints,_loc2_,_loc3_,this._enableRecap,this.userName);
            this.gui.updateUsernameText();
            _loc5_ = config.context.locale;
            _loc5_.translateDisplayObjects(LocaleCategory.GUI,this.gui as MovieClip,logger);
      ...
      public function showSurvivalInstallDialog() : void
      {
         this.showProductDialog("start_survival_not_installed_steam","start_survival_confirm_title");
      }
      
      public function showEternalArenaInstallDialog() : void
      {
         this.showProductDialog("start_eternal_arena_not_installed_steam","start_eternal_arena_confirm_title");
      }
      
      private function showEternalArenaPurchaseDialog() : void
      {
         this.showPurchaseDialog("dlc_eternal_arena",this.showEternalArenaUnavailableDialog);
      }
      
      private function showSurvivalPurchaseDialog() : void
      {
         this.showPurchaseDialog("dlc_survival",this.showSurvivalUnavailableDialog);
      }
      
      private function showPurchaseDialog(param1:String, param2:Function) : void
      {
         var _loc4_:SagaDlcEntry = null;
         var _loc3_:SagaDlcEntry = null;
         for each(_loc4_ in config.saga.def.dlcs.entries)
         {
            if(_loc4_.id == param1)
            {
               _loc3_ = _loc4_;
         ...
         if(_loc3_ != null && Boolean(SagaIap.impl))
         {
            if(SagaIap.impl.skipIngamePitch)
            {
               ...
               SagaIap.impl.purchaseProduct(_loc3_.id);
            }
            else
            {
               ...
               SagaIap.impl.requestProductInfo(_loc3_);
            }
         }
         else
         {
            if(_loc3_ != null)
            {
               if(SagaDlcEntry.dlcCheck.showStoreDlc(_loc3_))
               {
                  return;
               }
            }
      ...
      private function showEternalArenaUnavailableDialog() : void
      {
         this.showProductDialog("start_eternal_arena_unavailable_body","start_eternal_arena_confirm_title");
      }
      
      private function showSurvivalUnavailableDialog() : void
      {
         this.showProductDialog("start_survival_unavailable_body","start_survival_confirm_title");
      }
      
      private function showProductDialog(param1:String, param2:String) : void
      {
         var _loc3_:String = config.gameGuiContext.translate("ok");
         var _loc4_:String = config.gameGuiContext.translate(param2);
         var _loc5_:String = config.gameGuiContext.translate(param1);
         config.gameGuiContext.createDialog().openDialog(_loc4_,_loc5_,_loc3_);
      }
      
      public function guiSagaEternalArena() : void
      {
         if(!this._etar)
         {
            this._etar = new SagaStartPage_EternalArena(config);
         }
         this._etar.checkForDlc(this.showEternalArenaPurchaseDialog,this.showEternalArenaUnavailableDialog);
      }
      
      public function guiSagaSurvival() : void
      {
         this.checkForSurvivalDlc(this.startSurvival,this.showSurvivalPurchaseDialog,this.showSurvivalInstallDialog);
      }
      ...
      private function checkForSurvivalDlc(param1:Function, param2:Function = null, param3:Function = null) : void
      {
         ...
         if(dlc_survival)
         {
            if(this.checkSurvivalFileExists(saga) && ownedCallback != null)
            {
               ownedCallback();
            }
            return;
         ...
         entry.addEventListener(SagaDlcEvent.DLC_CHECK_COMPLETE,function(param1:SagaDlcEvent):void
         {
            ...
            {
               if(checkSurvivalFileExists(saga) && ownedCallback != null)
               {
                  ownedCallback();
               }
            }
      ...
      private function checkSurvivalFileExists(param1:Saga) : Boolean
      {
         if(!config.context.appInfo.checkFileExists(config.resman.assetPath,param1.def.survivalSagaUrl))
         {
            logger.e("CONFIG","ERROR: Invalid survival_saga URL: " + param1.def.survivalSagaUrl);
            return false;
         }
         return true;
      }
      ...
      public function startSurvival() : void
      {
         ...
         var yes:String = null;
         saga = config.saga;
         survivalUrl = saga.def.survivalSagaUrl;
         var hasPlayedSurvival:Boolean = saga.masterSave.hasChild(saga.def.survivalSagaKey);
