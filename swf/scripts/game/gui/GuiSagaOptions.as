package game.gui
{
   ...
   public class GuiSagaOptions extends GuiBase implements IGuiSagaOptions
   {
      ...
      private function updateVersionText() : void
      {
         ...
         var _loc4_:BuildInfo = null;
         this._text_version.text = "";
         if(Boolean(Ga.an) && Boolean(Ga.av))
         {
            _loc1_ = Ga.an + " " + Ga.av;
            _loc1_ = _loc1_.replace(/%20/g," ");
            this._text_version.text = _loc1_;
         }
         else
         {
            _loc2_ = !!context ? context.saga : null;
            _loc3_ = !!_loc2_ ? _loc2_.appinfo : null;
            _loc4_ = !!_loc3_ ? _loc3_.buildInfo : null;
            if(_loc4_)
            {
               this._text_version.text = _loc4_.id.toString();
            }
            else
            {
               this._text_version.text = "no idea...";
            }
         }
      }
      ...
      {
         var _loc1_:ISaga = context.iSaga;
         if(!_loc1_)
         {
            return;
         }
         var _loc2_:Boolean = SagaOptionsPage.ENABLE_SHARE_BAR;
         var _loc3_:Boolean = GuiSagaOptionsConfig.DEMO_MODE;
         this._share.visible = !_loc3_ && _loc2_ && !_loc1_.getVarBool(SagaVar.VAR_HIDE_SAGA_OPTIONS_SHARE);
         this._button$difficulty.visible = !_loc3_ && !_loc1_.isSurvival && !_loc1_.getVarBool(SagaVar.VAR_HIDE_SAGA_OPTIONS_DIFFICULTY) && !(_loc1_.isEternalArena && !_loc1_.isInBattle);
         this._button$credits.visible = !_loc3_ && !_loc1_.getVarBool(SagaVar.VAR_HIDE_SAGA_OPTIONS_CREDITS);
         this._button$load.visible = !_loc3_ && !_loc1_.isSurvival && !_loc1_.getVarBool(SagaVar.VAR_HIDE_SAGA_OPTIONS_LOAD) && !_loc1_.isEternalArena;
         this._button$opt_audio.visible = true;
         if(_loc3_)
         {
      ...
      private function onQuitGameClicked(param1:Object) : void
      {
         ...
         else if(_loc2_.eternalArena)
         {
            if(_loc2_.getBattleBoard())
            {
               _loc5_ = String(context.translate("ea_quit_battle_title"));
               _loc4_ = String(context.translate("ea_quit_battle_body"));
            }
            else
            {
               _loc5_ = String(context.translate("ea_quit_title"));
               _loc4_ = String(context.translate("ea_quit_body"));
            }
         }
         else
         {
