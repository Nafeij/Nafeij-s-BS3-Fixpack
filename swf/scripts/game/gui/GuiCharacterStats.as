package game.gui
{
   ...
   public class GuiCharacterStats extends MovieClip implements IGuiCharacterStats
   {
      ...
      public var _abilityButton:GuiStatButton;
         ...
         while(_loc1_ < numChildren)
         {
            ...
            if(_loc2_)
            {
               ...
               if(_loc2_.statType == null)
               {
                  ...
                  this._abilityButton.currentCharacter = this.currentCharacter;
               }
            }
            _loc1_++;
         }
      ...
      private function setStat(param1:StatType) : void
      {
         ...
         if(!this.currentCharacter.isSurvivalPromotable && !this.saga.isEternalArena)
         {
            _loc12_ = _loc13_ = 0;
         }
      ...
      private function handleAbilityVisibilityChanged(param1:Event) : void
      {
         ...
         if(this._abilityButton)
         {
            this._abilityButton.currentCharacter = this._abilityButton.currentCharacter;
         }
         var _loc3_:int = 0;
      ...
      {
         if(!this.context || !visible || !this.saga)
         {
            return false;
         }
         if(this.saga.isSurvival)
         {
            return this.attemptShowTipHeroesAbilitySelect();
         }
         return this.attemptShowTipHeroesAbility() || this.attemptShowTipHeroesPoints() || this.attemptShowTipHeroesTalent();
      }
      
      public function attemptShowTipHeroesAbilitySelect() : Boolean
      {
         if(this.tutorial_id || !visible || !this.saga)
         {
            return false;
         }
         ...
         if(!this.currentCharacter || !this.currentCharacter.selectAdditionalAbility)
         {
            return false;
         }
         var _loc1_:Boolean = this.context.saga.getVarBool(SagaVar.VAR_TUT_HEROES_ABILITY_SELECT);
         if(_loc1_)
         {
            return false;
         }
         this._showTipHeroesAbility();
         this.context.saga.setVar(SagaVar.VAR_TUT_HEROES_ABILITY_SELECT,true);
         return true;
      }
