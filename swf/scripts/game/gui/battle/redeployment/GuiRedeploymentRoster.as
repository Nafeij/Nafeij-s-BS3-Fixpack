package game.gui.battle.redeployment
{
   ...
   public class GuiRedeploymentRoster extends GuiCharacterIconSlots
   {
      ...
      private function initCharacterIconSlots(param1:IGuiContext, param2:Number, param3:Number) : void
      {
         ...
         var _loc6_:IGuiCharacterIconSlot = null;
         var _loc7_:IBattleEntity = null;
         var _loc8_:* = false;
         var _loc4_:int = 0;
         while(_loc4_ < this.roster.numCombatants)
         {
            _loc5_ = this.roster.getCombatantAt(_loc4_);
            if(!(param1.iSaga.isSurvival && (!_loc5_.isSurvivalRecruited || _loc5_.isSurvivalDead)))
            {
               _loc6_ = new this.redeploymentEntityFrameClazz() as IGuiCharacterIconSlot;
               _loc6_.init(param1);
               _loc6_.setCharacter(_loc5_,EntityIconType.INIT_ORDER);
               iconSlots.push(_loc6_);
               addChild(_loc6_ as DisplayObject);
               _loc7_ = this._battleParty.getMemberByDefId(_loc5_.id);
               _loc8_ = _loc7_ != null;
               if(!_loc8_)
               {
                  _loc7_ = this._bbRedeploy.getBattleEntity(_loc5_);
               }
               _loc6_.iconEnabled = true;
            }
            _loc4_++;
         }
      ...
      public function resizeHandler(param1:Number, param2:Number, param3:MovieClip) : void
      {
         ...
         var _loc4_:int = 0;
         while(_loc4_ < iconSlots.length && _loc4_ < this._iconPositions.length)
         {
      ...
      public function updateRosterIcons() : void
      {
         ...
         {
            _loc6_ = 0;
            while(_loc6_ < iconSlots.length && _loc6_ < this._iconPositions.length)
            {
