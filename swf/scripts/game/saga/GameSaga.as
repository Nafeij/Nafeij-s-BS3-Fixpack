package game.saga
{
   ...
   public class GameSaga extends Saga
   {
      ...
      override public function performSceneStart(param1:String, param2:String, param3:Boolean) : void
      {
         ...
         if(Boolean(def.eternalArena))
         {
            logger.error("GameSaga [" + def.url + "] no startscene expected for survival or EA mode");
            return;
         }
      ...
      override public function showSaveProfileLoad(param1:Function = null) : void
      {
         this.config.pageManager.showSaveProfileLoad(true,param1);
      }
      
      override public function showSaveProfileStart(param1:Function, param2:Function = null) : void
      {
         this.config.pageManager.showSaveProfileStart("start",param1,param2);
      }
      ...
      override protected function eternalArenaHandleStart() : void
      {
         ...
         if(SagaDef.ETERNAL_ARENA_ENABLED)
         {
            logger.info("GameSaga.eternalArenaHandleStart " + this);
            if(getVarBool("ea_start_lb"))
            {
               this.config.fsm.transitionTo(SagaEternalArenaLeaderboardsState,this.config.fsm.current.data);
            }
            else if(getVarBool("ea_start_arena_setup") && eternalArena && Boolean(eternalArena.currentArrangement))
            {
               setVar("ea_start_arena_setup",false);
               _loc1_ = Boolean(eternalArena.challenge) && eternalArena.challenge.arrangement == eternalArena.currentArrangement;
               this.config.fsm.current.data.setValue(GameStateDataEnum.EA_ARRANGEMENT,eternalArena.currentArrangement);
               this.config.fsm.current.data.setValue(GameStateDataEnum.EA_ARRANGEMENT_READONLY,_loc1_);
               this.config.fsm.transitionTo(SagaEternalArenaSetupState,this.config.fsm.current.data);
            }
            else
            {
               this.config.fsm.transitionTo(SagaEternalArenaStartState,this.config.fsm.current.data);
            }
         }
      }
      ...
      private function _dialogSurvivalRenownCloseHandler(param1:*) : void
      {
         ...
         this.config.pageManager.hideOptions();
