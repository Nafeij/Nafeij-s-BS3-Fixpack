package engine.core.gp
{
   ...
   public class GpBinder extends EventDispatcher
   {
      ...
      {
         if(Boolean(this.logger) && param1 in this.binds)
         {
            this.logger.debug("KeyBinder overriding " + param1 + " " + this.binds[param1] + " with " + param2);
         }
            ...
            if(_loc3_)
            {
               ...
               while(_loc4_ < _loc3_.length)
               {
                  ...
                  if(_loc5_.cmd == param1)
                  {
                     if(this.logger)
                     {
                        this.logger.info("GpBinder.unbind() Unbinding " + _loc5_.cmd + " from " + _loc2_);
                     }
