package engine.saga
{
   ...
   public class SagaShellCmdManager extends ShellCmdManager
   {
      ...
      public var ea:SagaEternalArenaShellCmdManager;
      ...
      public function SagaShellCmdManager(param1:ILogger, param2:Saga, param3:Boolean)
      {
         ...
         if(param3)
         {
            ...
            add("survival_recruit",this.shellFuncSurvivalRecruit,true);
            add("test_shatter",this.shellFuncTestShatter,true);
            add("cart",this.shellFuncCart,true);
            addAlias("carts","cart");
            add("master",this.shellFuncMasterVars,true);
            add("apply_dlc",this.shellFuncReapplyDlc,true);
            add("entity_kill_id",this.shellFuncCheckEntityKillId,true);
            add("resend_refereum",this.shellFuncResendRefereum,false);
            add("test_refereum",this.shellFuncTestRefereum,true);
            ...
            addShell("ab",this.ab);
            this.ea = new SagaEternalArenaShellCmdManager(param1,param2,param3);
            addShell("ea",this.ea);
         }
      }
      ...
      private function shellFuncMasterVars(param1:CmdExec) : void
      {
         ...
         {
            logger.info("List of Master Save values:");
            logger.info(this.saga.masterSave.getDebugString());
         }
      }
      ...
      private function shellFuncKills(param1:CmdExec) : void
      {
         ...
         if(this.saga)
         {
            this.saga.incrementMasterKillCountForEntity(_loc6_.eaKillCounterId,_loc4_);
         }
      }
      ...
      private function shellFuncReapplyDlc(param1:CmdExec) : void
      {
         this.saga.reapplyDlcs();
      }
      
      private function shellFuncTestRefereum(param1:CmdExec) : void
      {
         Refereum.completeAchievement("_acv_local");
      }
      
      private function shellFuncResendRefereum(param1:CmdExec) : void
      {
         this.saga.sendRefereumAllAchievementUnlocks(true);
      }
      
      private function shellFuncCheckEntityKillId(param1:CmdExec) : void
      {
         var _loc3_:IEntityDef = null;
         var _loc2_:Array = param1.param;
         if(_loc2_.length == 2)
         {
            _loc3_ = this.saga.def.cast.getEntityDefById(_loc2_[1]) as IEntityDef;
            logger.info("Entity [" + _loc3_.id + "]\'s kill counter id is [" + _loc3_.eaKillCounterId + "]");
         }
      }
      ...
      private function shellFuncSurvivalRecruit(param1:CmdExec) : void
      {
         var _loc2_:Array = null;
         var _loc4_:IEntityDef = null;
         var _loc5_:int = 0;
         if(!this.saga || !this.saga.isSurvival || !this.saga.roster)
         {
            logger.error("Error, command " + _loc2_[0] + " used with invalid saga!");
         }
         _loc2_ = param1.param;
         if(_loc2_.length < 2)
         {
         ...
         {
            _loc5_ = 0;
            while(_loc5_ < this.saga.roster.numEntityDefs)
            {
               _loc4_ = this.saga.roster.getEntityDef(_loc5_);
               if(!_loc4_.isSurvivalRecruited && !_loc4_.isSurvivalDead)
               {
                  _loc4_.isSurvivalRecruited = true;
                  logger.info("Recruiting character " + _loc4_.name);
                  this.saga.incrementGlobalVar(SagaVar.VAR_SURVIVAL_NUM_RECRUITS,1);
                  this.saga.setVar("survival_win_recruits_num",this.saga.getVarInt(SagaVar.VAR_SURVIVAL_NUM_RECRUITS));
               }
               _loc5_++;
            }
         }
         ...
         {
            _loc4_ = this.saga.roster.getEntityDefById(_loc3_);
            if(!_loc4_)
            {
               logger.info("Cast member " + _loc3_ + " not found!");
               return;
            }
            if(!_loc4_.isSurvivalRecruited && !_loc4_.isSurvivalDead)
            {
