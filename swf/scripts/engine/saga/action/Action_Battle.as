package engine.saga.action
{
   ...
   public class Action_Battle extends Action
   {
      ...
      override public function triggerAssembleHeroesComplete() : void
      {
         ...
         var _loc12_:int = 0;
         var _loc13_:BattleScenarioDef = null;
         ...
         var _loc8_:* = !def.use_waves;
         if(def.param)
         {
            def.param = def.param.replace(";",",");
            _loc9_ = new Vector.<String>();
            _loc11_ = def.param.split(",");
            _loc12_ = 0;
            while(_loc11_.length > _loc12_)
            {
         ...
         var _loc10_:SceneLoaderBattleInfo = new SceneLoaderBattleInfo();
         _loc10_.snap = this.snap;
         _loc10_.url = def.scene;
         _loc10_.happening = def.happeningId;
         _loc10_.battle_board_id = _loc4_;
         _loc10_.battle_deployment_area = _loc6_;
         _loc10_.battle_bucket_deployment = _loc3_;
         _loc10_.battle_spawn_tags = def.spawn_tags;
         _loc10_.battle_bucket_quota = _loc2_;
         _loc10_.battle_bucket = _loc1_;
         _loc10_.scenarioDef = null;
         _loc10_.music_url = def.battle_music;
         _loc10_.music_override = def.battle_music_override;
         _loc10_.war = def.war;
         _loc10_.sparring = def.battle_sparring;
         _loc10_.timer = def.battle_timer;
         _loc10_.wave_suppress = _loc8_;
         _loc10_.wave_bucket_overrides = _loc9_;
         if(def.battle_scenario_id)
         {
            _loc13_ = saga.getBattleScenarioDef(def.battle_scenario_id);
            _loc10_.scenarioDef = _loc13_;
            _loc13_.updateBattleInfo(_loc10_);
         }
         saga.performBattleStart(_loc10_,_loc5_);
      }
