package engine.battle.fsm
{
   ...
   public class BattleTurnOrder extends EventDispatcher implements IBattleTurnOrder
   {
      ...
      public function BattleTurnOrder(param1:ILogger, param2:ISaga, param3:IBattleBoard)
      {
         ...
         if(param2)
         {
            if(param2.isSurvival || param2.isEternalArenaPillage)
            {
               this._pillage = true;
      ...
      private function _resetTurnOrder_pillage_revised() : void
      {
         ...
         {
            if(Boolean(this._aliveOrder[_loc1_]) && !this._aliveOrder[_loc1_].alive)
            {
               this._aliveOrder.splice(_loc1_,1);
            }
            else
            {
               _loc1_++;
         ...
         while(_loc2_ < this._turnTeams.length)
         {
            ...
            while(_loc4_ < _loc3_.turnParties.length)
            {
               ...
               {
                  _loc7_ = false;
                  _loc8_ = _loc5_.members[_loc6_];
                  if(this._aliveOrder.indexOf(_loc8_) < 0)
                  {
                     _loc9_ = 1;
                     while(_loc9_ < this._aliveOrder.length)
                     {
                        _loc10_ = this._aliveOrder[_loc9_];
                        _loc11_ = this._aliveOrder[_loc9_ - 1];
                        if(_loc10_.team == _loc11_.team && _loc10_.team != _loc8_.team)
                        {
                           this._aliveOrder.splice(_loc9_,0,_loc8_);
                           _loc7_ = true;
                           break;
                        }
                        _loc9_++;
      ...
      public function resetTurnOrder(param1:Boolean = false) : void
      {
         ...
         else if(this.pillage)
         {
            if(param1)
            {
               this._resetTurnOrder_pillage_revised();
            }
            else
            {
      ...
      public function turnOrderNextTurn(param1:IBattleEntity) : IBattleEntity
      {
         ...
         if(this._pillage)
         {
            return this._turnOrderNextTurnPillage(param1);
         }
      ...
      public function _turnOrderNextTurnPillage(param1:IBattleEntity) : IBattleEntity
      {
         ...
         {
            _loc3_ = (this._index + 1) % this._turnTeams.length;
         ...
         else
         {
            ...
            if(_loc5_)
            {
               ...
               {
                  _loc2_ = this._aliveOrder.shift();
                  this._aliveOrder.push(_loc2_);
               }
               else
               {
                  _loc2_ = this._aliveOrder[0];
         ...
         this.updateTurnTeamFromActiveEntity(_loc2_);
         this.checkItOut();
         return _loc2_;
      }
      ...
      private function _fillAliveOrder() : void
      {
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         var _loc8_:BattleTurnTeam = null;
         var _loc9_:int = 0;
         var _loc10_:BattleTurnParty = null;
         var _loc11_:int = 0;
         var _loc12_:IBattleEntity = null;
         var _loc1_:int = 0;
         var _loc2_:int = this.computeMaxPartySize();
         var _loc3_:Boolean = this.saga.isSurvival;
         var _loc4_:Boolean = this.saga.isEternalArenaPillage;
         var _loc5_:int = 0;
         while(_loc5_ < _loc2_)
         {
            _loc6_ = 0;
            while(_loc6_ < this._turnTeams.length)
            {
               _loc7_ = (_loc6_ + this._index) % this._turnTeams.length;
               _loc8_ = this._turnTeams[_loc7_];
               _loc9_ = 0;
               for(; _loc9_ < _loc8_.turnParties.length; _loc9_++)
               {
                  _loc10_ = _loc8_.turnParties[_loc9_];
                  _loc11_ = _loc5_;
                  if(!_loc3_ && !_loc4_)
                  {
                     if(_loc7_ != this._index)
                     {
                        _loc11_++;
                     }
                  }
                  if(this._pillage)
                  {
                     if(_loc11_ >= _loc10_.members.length)
                     {
                        continue;
                     }
                  }
                  _loc12_ = _loc10_.getFutureCurrent(_loc11_);
                  if(_loc12_)
                  {
                     this._aliveOrder.push(_loc12_);
                  }
                  else
                  {
                     this.logger.info("BattleTurnOrder.checkDirty unable to getFutureCurrent " + _loc11_ + " for " + _loc10_);
                  }
               }
               _loc6_++;
            }
      ...
      public function removeEntity(param1:IBattleEntity) : void
      {
         ...
         if(!_loc2_ && Boolean(this._turnTeams.length))
         {
