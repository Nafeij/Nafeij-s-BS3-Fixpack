package engine.battle.fsm.state
{
   ...
   public class BattleStateWaveRedeploy_Assemble extends BattleStateWave_Base implements IBattleStateUserDeploying
   {
      ...
      override protected function handleEnteredState() : void
      {
         ...
         if(board.getSaga().isEternalArena)
         {
            return;
         }
      ...
      {
         super.update(param1);
         if(BattleFsmConfig.guiHudEnabled || BattleFsmConfig.guiWaveDeployEnabled)
         {
            advanceTimer(param1);
         }
      }
      
      public function get isLocalDeployed() : Boolean
      {
         return false;
      }
      
      public function set isLocalDeployed(param1:Boolean) : void
      {
      }
      
      override protected function handleTimeout() : void
      {
         logger.info("BattleStateWaveRedeploy_Assemble force timeout deploy");
         this.redeployComplete();
      }
   }
}
