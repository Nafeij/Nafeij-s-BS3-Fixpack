   ...
   public class BattleFsm extends Fsm implements IBattleFsm
   {
      ...
      public function BattleFsm(param1:Session, param2:IBattleBoard, param3:ILogger, param4:BattleFsmConfig, param5:BattleCreateData, param6:int, param7:Boolean, param8:Chat, param9:Fsm)
      {
         ...
         if(!this._isOnline)
         {
            ...
            if(Boolean(_loc10_) && (_loc10_.isSurvival || _loc10_.isEternalArena && _loc10_.eternalArena.isTimedBattle))
            {
         ...
         if(!stopping && Boolean(current))
         {
            ...
            if(currentClass == BattleStateFinish || currentClass == BattleStateFinished)
            {
               ...
               if(_loc3_)
               {
                  ...
                  _loc3_.triggerBattleFinallyFinished(_loc4_,param1);
               }
               return;
            }
            _loc2_ = current.data;
      ...
      public function killall() : Boolean
      {
         ...
         var _loc11_:Saga = null;
         var _loc12_:int = 0;
         var _loc13_:int = 0;
         var _loc14_:IBattleEntity = null;
         var _loc15_:Stats = null;
         if(this.isOnline)
         {
            return false;
         }
         ...
         if(_loc2_)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               _loc5_ = _loc1_.party;
               _loc11_ = Saga.instance;
               _loc12_ = 0;
               while(_loc12_ < _loc9_)
               {
                  _loc13_ = _loc10_.nextMinMax(0,_loc5_.numMembers - 1);
                  _loc14_ = _loc5_.getMember(_loc13_);
                  _loc15_ = _loc14_.def.stats;
                  _loc15_.setBase(StatType.KILLS,_loc15_.getBase(StatType.KILLS) + 1);
                  if(_loc11_)
                  {
                     _loc11_.incrementMasterKillCountForEntity(_loc14_.def.eaKillCounterId);
                  }
                  _loc12_++;
               }
            }
