   ...
   public class BattleParty extends EventDispatcher implements IBattleParty
   {
      ...
      public function set deployed(param1:Boolean) : void
      {
         ...
         if(param1)
         {
            ...
            {
               _loc4_ = this.members[_loc3_];
               if(!_loc4_.deploymentReady)
               {
                  this._logger.error("BattleParty.set deployed(" + param1 + ") - Failed to deploy " + _loc4_.id + " removing from party.");
                  this.board.removeEntity(_loc4_);
                  _loc4_.cleanup();
               }
               else
               {
