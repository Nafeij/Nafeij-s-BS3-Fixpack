package engine.battle.entity.model
{
   ...
   public class BattleEntity extends Entity implements IBattleEntity, IAnimControllerListener
   {
      ...
      private function temporaryCharacterCtor() : void
      {
         ...
         if(_loc1_)
         {
            ...
            if(item)
            {
               if(!Saga.instance || !Saga.instance.battleItemsDisabled)
               {
                  ...
                  if(this.saga && this.saga.isSurvival && this.isPlayer)
                  {
                     this.saga.setVar(SagaVar.VAR_SURVIVAL_ITEM_EQUIPPED,1);
                  }
               }
            }
            if(this.titleItem)
         ...
         this.wilstat = stats.getStat(StatType.WILLPOWER,false);
         if(this.wilstat)
         {
            this.wilstat.addEventListener(StatEvent.CHANGE,this.willChangeHandler);
         }
      }
      ...
      public function cleanupTemporaryCharacter() : void
      {
         ...
         if(this.wilstat)
         {
            this.wilstat.removeEventListener(StatEvent.CHANGE,this.willChangeHandler);
            this.wilstat = null;
         }
      ...
      {
         var _loc3_:BattleEntityEvent = null;
         if(cleanedup)
         {
            return;
         }
         ...
         {
            _loc3_ = new BattleEntityEvent(BattleEntityEvent.DAMAGED,this);
            _loc3_.statType = _loc2_.type;
            _loc3_.amount = -param1.delta;
            dispatchEvent(_loc3_);
            if(this.saga && this.saga.isEternalArena && this.saga.isInBattle && param1.type == StatEvent.CHANGE && this.isEnemy)
            {
               this.saga.incrementGlobalVar(Saga.VAR_PRG_EA_STR_DMG_GIVEN,Math.abs(param1.delta));
            }
         }
         if(_loc2_.value <= 0)
         {
      ...
      {
         var _loc2_:BattleEntityEvent = null;
         if(!this.armstat)
         {
            return;
         }
         ...
         {
            _loc2_ = new BattleEntityEvent(BattleEntityEvent.DAMAGED,this);
            _loc2_.statType = StatType.ARMOR;
            _loc2_.amount = -param1.delta;
            dispatchEvent(_loc2_);
            if(this.saga && this.saga.isEternalArena && this.saga.isInBattle && param1.type == StatEvent.CHANGE)
            {
               if(this.isEnemy)
               {
                  this.saga.incrementGlobalVar(Saga.VAR_PRG_EA_ARM_DMG_GIVEN,Math.abs(param1.delta));
               }
               else if(this.isPlayer)
               {
                  this.saga.incrementGlobalVar(Saga.VAR_PRG_EA_ARM_DMG_TAKEN,Math.abs(param1.delta));
               }
            }
         }
      }
      
      protected function willChangeHandler(param1:StatEvent) : void
      {
         if(!this.wilstat)
         {
            return;
         }
         if(this.saga && this.saga.isEternalArena && this.saga.isInBattle && param1.type == StatEvent.CHANGE && param1.delta < 0 && this.isPlayer)
         {
            this.saga.incrementGlobalVar(Saga.VAR_PRG_EA_WIL_SPENT,Math.abs(param1.delta));
         }
      }
      ...
      public function onStartTurn() : void
      {
         ...
         dispatchEvent(new BattleEntityEvent(BattleEntityEvent.TURN_STARTED,this));
      }
      ...
      public function decrementIgnoreTargetRotation() : void
      {
         ...
         {
            this.logger.debug("BattleEntity::decrementIgnoreTargetRotation - Decremented below 0, while this won\'t cause any problems, it might be worth investigating what happened.");
            return;
         }
         --this._ignoreTargetRotation;
      }
