package engine.entity.def
{
   ...
   import engine.saga.ea.EternalArenaCastModifierTalentWeight;
   import engine.saga.ea.EternalArenaCastModifierWeight;
   import engine.saga.ea.EternalArenaCastModifiers;
   ...
   public class EntityDef extends EventDispatcher implements IEntityDef
   {
      ...
      protected var _selectAdditionalAbility:Boolean;
      
      protected var _cachedSelectAdditionalAbility:Boolean;
      ...
      protected var _eaKillCounterOverride:String;
      ...
      public function get selectAdditionalAbility() : Boolean
      {
         return this._selectAdditionalAbility && this.stats.rank >= 7;
      }
      
      public function set selectAdditionalAbility(param1:Boolean) : void
      {
         this._selectAdditionalAbility = param1;
      }
      
      public function get eaKillCounterOverride() : String
      {
         return this._eaKillCounterOverride;
      }
      
      public function set eaKillCounterOverride(param1:String) : void
      {
         this._eaKillCounterOverride = param1;
      }
      
      public function get eaKillCounterId() : String
      {
         return this._eaKillCounterOverride != null && this._eaKillCounterOverride != "" ? this._eaKillCounterOverride : this._id;
      }
      ...
      public function doApplyRankLimits(param1:int, param2:int, param3:UnitStatCosts, param4:ILogger) : void
      {
         var _loc9_:int = 0;
         var _loc10_:int = 0;
         var _loc11_:IAbilityDefLevel = null;
         var _loc5_:StatRange = this.statRanges.getStatRange(StatType.RANK);
         if(_loc5_ == null)
         {
            return;
         }
         if(param2 < this._stats.rank)
         {
            this._stats.rank = param2;
         }
         var _loc6_:int = Math.min(param1,_loc5_.max);
         if(_loc6_ > this._stats.rank)
         {
            if(param4)
            {
               param4.info("Force-ranking [" + this.id + "] from " + this.stats.rank + " to " + _loc6_);
            }
            this._stats.rank = _loc6_;
            if(_loc6_ > 1)
            {
               _loc9_ = this.stats.getBase(StatType.KILLS);
               _loc10_ = param3.getKillsRequiredToPromote(_loc6_ - 1);
               if(_loc9_ < _loc10_)
               {
                  this._stats.setBase(StatType.KILLS,_loc10_);
               }
            }
         }
         var _loc7_:int = 0;
         ...
         _loc7_ = 0;
         while(_loc7_ < this.actives.numAbilities)
         {
            _loc8_.push(this.actives.getAbilityDef(_loc7_).id);
            _loc7_++;
         }
         _loc7_ = 0;
         while(_loc7_ < _loc8_.length)
         {
            _loc11_ = this.actives.getAbilityDefLevelById(_loc8_[_loc7_]);
            if(_loc11_.rankAcquired > this._stats.rank)
            {
               this.actives.removeAbility(_loc11_.id);
            }
            else
            {
               this.actives.setAbilityDefLevel(_loc11_.def,Math.max(1,Math.min(3,(this._stats.rank - _loc11_.rankAcquired + 2) / 2)),_loc11_.rankAcquired,param4);
            }
            _loc7_++;
         }
      }
      
      public function spendPoints(param1:EternalArenaCastModifiers, param2:EntitiesMetadata, param3:UnitStatCosts, param4:TalentDefs) : void
      {
         var _loc5_:int = this.getMaxUpgrades(param2,param3);
         var _loc6_:int = _loc5_ - this.spentPoints;
         _loc6_ = this.upStats(param1,_loc6_);
         this.upTalents(param1,param4,_loc6_);
      }
      
      public function get respecAvailable() : Boolean
      {
         return this.spentPoints > 0 || this._cachedSelectAdditionalAbility && !this.selectAdditionalAbility || this.stats.titleRank > 0;
      }
      
      public function get spentPoints() : int
      {
         return this.stats.GetTotalUpgrades(this.statRanges) + (!!this.talents ? this.talents.totalRanks : 0);
      }
      
      private function upStats(param1:EternalArenaCastModifiers, param2:int) : int
      {
         var _loc4_:EternalArenaCastModifierWeight = null;
         ...
         var _loc3_:Vector.<EternalArenaCastModifierWeight> = this.getStatWeights(param1,Stats.userChangeStatVect);
         if(_loc3_ == null || _loc3_.length == 0)
         {
            return param2;
         }
         while(Boolean(_loc3_.length) && param2 > 0)
         {
            _loc3_.sort(EternalArenaCastModifierWeight.sort);
            _loc4_ = _loc3_[0];
            _loc5_ = this.stats.getStat(_loc4_.type);
            _loc6_ = this.statRanges.getStatRange(_loc4_.type);
            if(_loc5_.value >= _loc6_.max)
            {
               _loc3_.splice(0,1);
            }
            else
         ...
         return this.upStats(param1,param2);
      }
      
      private function getStatWeights(param1:EternalArenaCastModifiers, param2:Vector.<StatType>) : Vector.<EternalArenaCastModifierWeight>
      {
         var _loc5_:StatType = null;
         var _loc6_:EternalArenaCastModifierWeight = null;
         var _loc7_:Stat = null;
         var _loc8_:StatRange = null;
         var _loc3_:Vector.<EternalArenaCastModifierWeight> = new Vector.<EternalArenaCastModifierWeight>();
         var _loc4_:int = 0;
         while(_loc4_ < param2.length)
         {
            _loc5_ = Stats.userChangedStatTypes[_loc4_];
            _loc6_ = param1.getStatWeight(this.id,_loc5_);
            _loc7_ = this.stats.getStat(_loc6_.type);
            _loc8_ = this.statRanges.getStatRange(_loc6_.type);
            if(_loc7_.value < _loc8_.max)
            {
               if(Boolean(_loc6_) && _loc6_.weight > 0)
               {
      ...
      private function upTalents(param1:EternalArenaCastModifiers, param2:TalentDefs, param3:int) : int
      {
         var _loc5_:EternalArenaCastModifierTalentWeight = null;
         var _loc6_:TalentDef = null;
         var _loc7_:Talent = null;
         if(param3 <= 0)
         {
            return 0;
         }
         var _loc4_:Vector.<EternalArenaCastModifierTalentWeight> = this.getTalentWeights(param1,param2);
         if(_loc4_ == null || _loc4_.length == 0)
         {
            return param3;
         }
         while(Boolean(_loc4_.length) && param3 > 0)
         {
            _loc4_.sort(EternalArenaCastModifierTalentWeight.sort);
            _loc5_ = _loc4_[0];
            _loc6_ = param2.getDefById(_loc5_.name);
            _loc7_ = this.talents.getTalentByParentStatType(_loc6_.parentStatType);
            if(Boolean(_loc7_) && _loc7_.def != _loc6_)
            {
               _loc4_.splice(0,1);
            }
            else
            {
               if(!_loc7_)
               {
                  _loc7_ = this.talents.addTalent(new Talent(_loc6_,0),false);
               }
               if(_loc7_.rank >= param2.maxAvailableRanks)
               {
                  _loc4_.splice(0,1);
               }
               else
         ...
         if(this.talents)
         {
            this.talents.computeTotalRanks();
         }
         return this.upTalents(param1,param2,param3);
      }
      
      private function getTalentWeights(param1:EternalArenaCastModifiers, param2:TalentDefs) : Vector.<EternalArenaCastModifierTalentWeight>
      {
         var _loc6_:TalentDef = null;
         var _loc7_:EternalArenaCastModifierTalentWeight = null;
         var _loc8_:Talent = null;
         var _loc9_:StatType = null;
         var _loc3_:Vector.<TalentDef> = param2.defs;
         var _loc4_:Vector.<EternalArenaCastModifierTalentWeight> = new Vector.<EternalArenaCastModifierTalentWeight>();
         if(!this.talents)
         {
            this.talents = new Talents(this.id + "_EA_orig");
         }
         var _loc5_:int = 0;
         for(; _loc5_ < _loc3_.length; _loc5_++)
         {
            _loc6_ = _loc3_[_loc5_];
            _loc7_ = param1.getTalentWeight(this.id,_loc6_.id);
            if(!(_loc7_ == null || _loc7_.weight <= 0))
            {
               _loc8_ = this.talents.getTalentById(_loc6_.id);
               if(_loc8_ == null)
               {
                  _loc9_ = param2.getDefById(_loc6_.id).parentStatType;
                  if(this.talents.getTalentByParentStatType(_loc9_) != null)
                  {
                     continue;
                  }
               }
               else if(_loc8_.rank >= param2.maxAvailableRanks)
               {
                  continue;
               }
               _loc4_.push(_loc7_.clone());
            }
         }
         return _loc4_;
      }
      
      public function get maxEATitleRank() : uint
      {
         return Math.max(this.stats.rank - 11,0);
      }
      
      public function doFullRespec(param1:ISaga, param2:Boolean = true) : void
      {
         var _loc6_:Talent = null;
         var _loc3_:ILegend = param1.icaravan.legend;
         var _loc4_:int = 0;
         var _loc5_:Stat = null;
         _loc4_ = 0;
         while(_loc4_ < Stats.userChangedStatTypes.length)
         {
            _loc5_ = this.stats.getStat(Stats.userChangedStatTypes[_loc4_]);
            _loc5_.base = this.statRanges.getStatRange(_loc5_.type).min;
            if(this.talents)
            {
               _loc6_ = this.talents.getTalentByParentStatType(_loc5_.type);
               if(_loc6_)
               {
                  this.talents.removeTalent(_loc6_);
               }
            }
            _loc4_++;
         }
         ...
         if(this._cachedSelectAdditionalAbility)
         {
            this.selectAdditionalAbility = true;
            _loc4_ = 0;
            while(_loc4_ < this.additionalActives.length)
            {
               this.actives.removeAbility(this.additionalActives[_loc4_]);
               _loc4_++;
            }
         }
         if(this.defTitle)
         {
            if(param2)
            {
               param1.unConsumeTitle(this.defTitle);
            }
            this.stats.titleRank = 0;
            this.defTitle = null;
         }
         if(this.defItem)
         {
            this.defItem.owner = null;
            if(param2)
            {
               _loc3_.items.addItem(this.defItem);
            }
            this.defItem = null;
         }
      }
