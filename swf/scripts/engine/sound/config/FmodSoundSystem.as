package engine.sound.config
{
   ...
   public class FmodSoundSystem extends EventDispatcher implements ISoundSystem, ISoundDefBundleListener
   {
      ...
      public function set music(param1:ISound) : void
      {
         ...
         if(Boolean(this._music) && Boolean(param1))
         {
            if(this._music.def.eventName == param1.def.eventName)
            {
               if(this._music.playing && !this._music.def.allowDuplicateSounds)
               {
                  return;
               }
            }
      ...
      public function refereumInitialize(param1:String, param2:String, param3:String, param4:Boolean) : void
      {
         this.driver.refereumInitialize(param1,param2,param3,param4);
      }
   }
}
