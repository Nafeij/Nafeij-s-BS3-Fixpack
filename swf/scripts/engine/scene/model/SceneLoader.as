      ...
      private function loadFromDef() : void
      {
         ...
         try
         {
            ...
            if(board_id)
            {
               ...
               {
                  b = this.scene.prepareBoard(board_id,this.battle_info.scenarioDef,this.isOnline,this.battle_info.battle_spawn_tags,this.battle_info.eaScorecard);
                  if(this.setupLocalPartyCallback != null)
                  {
                     this.setupLocalPartyCallback(b,this.battle_info.battle_deployment_area);
                  }
                  if(b && b.saga.isEternalArena && b.waves && this.battle_info.wave_suppress)
                  {
                     b.waves.cleanup();
                     b._waves = null;
                  }
                  if(this.battle_info.wave_bucket_overrides && this.battle_info.wave_bucket_overrides.length > 0 && Boolean(b.waves))
                  {
                     b.waves.overrideBuckets(this.battle_info.wave_bucket_overrides);
                  }
                  bs = b._spawn;
                  if(this.opponent)
                  {
                  ...
                  if(b.saga.isEternalArena && Boolean(b.saga.eternalArena.currentArrangement))
                  {
                     if(b.saga.eternalArena.currentArrangement.hasValue(EternalArenaOptionGroupTypes.RULES,EternalArenaChallenge.RNG_SEED))
                     {
                        iSeed = parseInt(b.saga.eternalArena.currentArrangement.getValue(EternalArenaOptionGroupTypes.RULES,EternalArenaChallenge.RNG_SEED));
                        b.saga.overrideRNGSeed(iSeed);
                        this.scene.context.resetRNG();
                     }
                  }
                  b.enabled = true;
                  if(b.error)
