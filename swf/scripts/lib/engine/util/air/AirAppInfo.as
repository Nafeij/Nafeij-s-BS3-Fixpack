package lib.engine.util.air
{
   ...
   public class AirAppInfo extends AppInfo
   {
      ...
      public function AirAppInfo(param1:Sprite, param2:BuildInfo, param3:int)
      {
         var root:Sprite = param1;
         var buildInfo:BuildInfo = param2;
         var ordinal:int = param3;
         super(root,buildInfo,ordinal);
      ...
      {
         return new AirAppInfo(root,buildInfo,param1);
      }
      ...
      override public function listDirectory(param1:String, param2:String, param3:String = "FILE_SORT_NONE") : Array
      {
         var _loc8_:File = null;
         var _loc9_:String = null;
         var _loc4_:File = this.processPath(param1,param2);
         if(!_loc4_.exists || !_loc4_.isDirectory)
         {
            return [];
         }
         ...
         switch(param3)
         {
            case AppInfo.FILE_SORT_LAST_MODIFIED:
               _loc5_.sort(this.sortOnLastModified);
               break;
            case AppInfo.FILE_SORT_LAST_MODIFIED_DESCENDING:
               _loc5_.sort(this.sortOnLastModified,Array.DESCENDING);
         }
         var _loc6_:Array = [];
         var _loc7_:* = _loc4_.url + "/";
         if(logger.isDebugEnabled)
         {
            logger.debug("AirAppInfo.listDirectory pfx=[" + _loc7_ + "]");
         }
         for each(_loc8_ in _loc5_)
         {
            _loc9_ = StringUtil.stripPrefix(_loc8_.url,_loc7_);
            _loc6_.push(_loc9_);
         }
         return _loc6_;
      }
      
      private function sortOnLastModified(param1:File, param2:File) : Number
      {
         var _loc3_:Number = Number(param1.modificationDate.getTime());
         var _loc4_:Number = Number(param2.modificationDate.getTime());
         if(_loc3_ > _loc4_)
         {
            return 1;
         }
